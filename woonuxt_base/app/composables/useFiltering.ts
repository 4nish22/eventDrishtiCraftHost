...
export function useFiltering() {
  ...
  function filterProducts(products: any[]): any[] {
    return products.filter((product) => {
      ...
      const categoryCondition = category.length ? product.productCategories?.nodes?.find((node: any) => category.includes(node.slug as string)) : true;

      // Tag filter
      const tag = getFilter('tag') || []; // ["best-selling"]
      const tagCondition = tag.length ? product.terms?.nodes?.find((node: any) => node.taxonomyName === 'product_tag' && tag.includes(node.slug)) : true;
      ...
      return ratingCondition && priceCondition && attributeCondition && categoryCondition && tagCondition && saleItemsOnlyCondition;
    });
  }
